...
const HAIR_FILLER_COUNT = 0
const MAX_ROT_Y = 0.35
const MAX_ROT_X = 0.25
const HEAD_SCALE = 0.65
// Reduced to ~3% sensitivity so scroll animation completes much faster
...
function Head({ scrollRef }: HeadProps) {
  ...
  // Hair ONLY on deep back of head — threshold tightened to z < -0.65
  const hairPuffs = useMemo(() => {
    const puffs: { pos: [number, number, number]; r: number; tone: number }[] = []

    // Anchors pushed further back (all z <= -0.7) to guarantee no face hair
    ...
    for (const [x, y, z, r] of anchors) {
      // Extra guard — only deep back
      if (z < -0.65) {
        puffs.push({ pos: [x, y, z], r, tone: 0.95 + Math.random() * 0.06 })
      }
    }

    // Filler curls — strictly z < -0.65
    for (let i = 0; i < 25; i++) {
      ...
      const z = -(radius * Math.sin(phi) * Math.sin(theta) + 0.75) // Pushed deeper
    ...
    // Tight curls on back top — z < -0.65 only
    for (let i = 0; i < 12; i++) {
      const a = (i / 12) * Math.PI * 2
      const r = 0.5 + Math.random() * 0.15
      const puffZ = -Math.sin(a) * r * 0.85 - 0.3 // Bias further back

      if (puffZ > -0.65) continue

      puffs.push({
        pos: [Math.cos(a) * r, 1.08 + Math.random() * 0.08, puffZ],
  ...
  useFrame((state, delta) => {
    ...
    mouseTarget.current.x = THREE.MathUtils.damp(mouseTarget.current.x, px, 4 * ANIMATION_SPEED, dt)
    mouseTarget.current.y = THREE.MathUtils.damp(mouseTarget.current.y, py, 4 * ANIMATION_SPEED, dt)

    if (group.current) {
      const targetRotY = THREE.MathUtils.clamp(
        scroll * Math.PI * 0.4 * SCROLL_SENSITIVITY,
        -MAX_ROT_Y * 0.04, // Tighter limit — animation finishes in ~3% of scroll range
        MAX_ROT_Y * 0.04,
      )
      const targetZ = -0.15 + scroll * 0.005 // Reduced z movement too
      group.current.rotation.y = THREE.MathUtils.damp(group.current.rotation.y, targetRotY, 2.5, dt)
      group.current.position.z = THREE.MathUtils.damp(group.current.position.z, targetZ, 2.5, dt)
    }

    if (headPivot.current) {
      const targetY = THREE.MathUtils.clamp(mouseTarget.current.x * 0.35, -MAX_ROT_Y, MAX_ROT_Y)
      const targetX = THREE.MathUtils.clamp(-mouseTarget.current.y * 0.25, -MAX_ROT_X, MAX_ROT_X)
      headPivot.current.rotation.y = THREE.MathUtils.damp(headPivot.current.rotation.y, targetY, 3.5, dt)
      headPivot.current.rotation.x = THREE.MathUtils.damp(headPivot.current.rotation.x, targetX, 3.5, dt)
    }

    const eyeRot = mouseTarget.current.x * 0.25
    const eyeTilt = -mouseTarget.current.y * 0.16
    if (leftEye.current) {
      leftEye.current.rotation.y = THREE.MathUtils.damp(leftEye.current.rotation.y, eyeRot, 6, dt)
      leftEye.current.rotation.x = THREE.MathUtils.damp(leftEye.current.rotation.x, eyeTilt, 6, dt)
    }
    if (rightEye.current) {
      rightEye.current.rotation.y = THREE.MathUtils.damp(rightEye.current.rotation.y, eyeRot, 6, dt)
      rightEye.current.rotation.x = THREE.MathUtils.damp(rightEye.current.rotation.x, eyeTilt, 6, dt)
    }

    if (handRef.current) {
      const hx = THREE.MathUtils.damp(handRef.current.position.x, handTarget.current, 6, dt)
  ...
  const skin = <meshStandardMaterial color={skinColor} roughness={0.55} metalness={0.01} />
  const skinNose = <meshStandardMaterial color="#d89565" roughness={0.58} />
  const cheekMat = <meshStandardMaterial color="#e88a6a" roughness={0.75} transparent opacity={0.6} />
  ...
  return (
    <group ref={group} position={[0, -0.05, 0]}>
      <group ref={headPivot} scale={HEAD_SCALE}>
        ...
        {/* Back-of-head hair only — all puffs are guaranteed z < -0.65 */}
        {hairPuffs.map((p, i) => (
